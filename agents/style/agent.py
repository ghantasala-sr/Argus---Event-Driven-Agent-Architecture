...
class StyleAgent:
    ...
    def _analyze_chunk(self, chunk: DiffChunk) -> tuple[list[Finding], BedrockResponse]:
        ...
        response = self.bedrock.invoke(prompt=prompt, system_prompt=STYLE_SYSTEM_PROMPT, temperature=0.1, max_tokens=2048)
        ...
        try:
            ...
            for raw in data.get("findings", []):
                ...
                findings.append(Finding(
                    severity=sev, category=raw.get("category", "style"),
                    file=raw.get("file", "unknown"), line=raw.get("line", 0),
                    message=raw.get("message", ""), suggestion=raw.get("suggestion", ""), agent="style"
                ))
        except Exception:
            pass
        return findings
